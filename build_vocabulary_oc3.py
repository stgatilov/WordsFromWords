...
frequences = {}
xmlfile = open('annot.opcorpora.xml', 'rb')
for event, token in ET.iterparse(xmlfile, tag = 'token'):
    ids = set()
    for version in token.find('tfr').iterfind('v'):
        ids.add(version.find('l').attrib['id'])
    if len(ids) == 1:
        lid = list(ids)[0]
